...
xdisp = 10 # prescribed displacement BC
...
for i in NodeintLabel:
   ...
   # if NodeCoord[i, 1] ==0.0:
    ...
    if NodeCoord[i, 0] == 20.0: #and NodeCoord[i, 1] == 0.0 :
        DisplacewithBC[2*i, 0] = xdisp
...
UnknownBCnodes = np.setxor1d(UnknownBCnodes, LoadBCnodes)
  #  if NodeCoord[i, 1] ==10.0:
   #     DisplacewithBC[2*i+1, 0] = 0
def DeAssemblyGlobalMatrix(kglobal, rglobal):
...
kreduced, rreduced = DeAssemblyGlobalMatrix(kglobal, rglobal)
...
contr = plt.tricontourf(DeformedCoord[:,0], DeformedCoord[:,1], ElemData-1, Dispx, 40, cmap='coolwarm' )
