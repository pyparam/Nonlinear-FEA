...
def DeAssemblyGlobalMatrix(kglobal, rglobal, LoadBCnodes, SupportBCnodes, xdisp):
...
def SolveU(kreduced, rreduced, UnknownBCnodes, LoadBCnodes, SupportBCnodes, xdisp):
    unknownU = np.linalg.solve(kreduced,rreduced)
    return unknownU
def assembleU(DisplacewithBC,UnknownBCnodes, LoadBCnodes, SupportBCnodes, unknownU, xdisp):
...
#def InitalDisp(DisplacewithBC):
 #   DispVec = np.linspace(0, xdisp, 11)
  #  Uincre = DispVec[1]-DispVec[0]
   # DispMat = DispVec*DisplacewithBC   
    #return DispMat, Uincre
  #  if NodeCoord[i, 1] ==10.0:
   #     DisplacewithBC[2*i+1, 0] = 0
#DispMat, diffU = InitalDisp(DisplacewithBC)
...
for idisp in range(0, 1):
    ...
    kreduced, rreduced = DeAssemblyGlobalMatrix(kglobal, rglobal, LoadBCnodes, SupportBCnodes, xdisp)
    solvedU =SolveU(kreduced, rreduced, UnknownBCnodes, LoadBCnodes, SupportBCnodes, xdisp)
    #solvedU = solvedU.reshape(len(solvedU), 1)
    tempsolvedU = np.zeros(len(solvedU))#.reshape(len(solvedU), 1)
    for iconv in range(5):
        convcriteria = (sum(rglobal**2))/(1+ sum(rextglobal**2))
        print(convcriteria)
        tempsolvedU = tempsolvedU + solvedU
        #print(du)
        du = assembleU(DisplacewithBC,UnknownBCnodes, LoadBCnodes, SupportBCnodes, tempsolvedU, xdisp)
        du = du.reshape(len(du))
       # print(du)
        kglobal, rglobal, rext = NlGlobalAssembly(ElemData, NodeData, LoadNode, E, nu, rextglobal, kglobal, du)
        kreduced, rreduced = DeAssemblyGlobalMatrix(kglobal, rglobal, LoadBCnodes, SupportBCnodes, xdisp)
   ...
   # print(du)
    #print('\n')
tempsolvedU = tempsolvedU + solvedU
Displacement = assembleU(DisplacewithBC,UnknownBCnodes, LoadBCnodes, SupportBCnodes, tempsolvedU, xdisp)   
...
#plt.tricontourf(DeformedCoord[:,0], DeformedCoord[:,1], ElemData-1, Dispx, 40, cmap='coolwarm' )
